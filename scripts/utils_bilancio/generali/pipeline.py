...
from utils_bilancio.chart_generation.confronti_europa import plot_peer_comparison
from utils_bilancio.chart_generation.confronti_oecd import (
...
from utils_bilancio.chart_generation.italia_entrate import (
...
from utils_bilancio.chart_generation.italia_patrimonio import plot_household_wealth_distribution
from utils_bilancio.chart_generation.italia_uscite import plot_cofog_spending, plot_total_spending_italy
from utils_bilancio.chart_generation.regioni import (
...
from utils_bilancio.generali.data_extraction import (
    ...
    load_peer_spending,
    load_peer_spending_functions_history,
    load_peer_spending_history,
    load_peer_spending_functions,
    load_peer_tax_pressure,
    load_peer_tax_pressure_history,
    load_tax_pressure,
    load_total_spending_italy,
)
from utils_bilancio.generali.exporter import export_bilancio_source_json
from utils_bilancio.regioni.bilanci import (
    SOURCE_OPENBDAP_REGIONI,
    append_regional_budgets_to_source_json,
    load_regional_budgets,
)
from utils_bilancio.regioni.normalizzazione import (
    append_regional_normalization_to_source_json,
    enrich_regional_budgets,
)
from utils_bilancio.italia.aggiustamenti import (
    append_spending_adjustments_to_source_json,
    augment_spending_frame,
    augment_total_spending_frame,
    load_spending_adjustments,
)
from utils_bilancio.regioni.siope_flussi import append_siope_flows_to_source_json, load_siope_flows
...
from utils_bilancio.generali.utils import (
...
def run(refresh=False):
    ...
    cofog_spending_trend, cofog_spending_trend_updated = load_cofog_spending_trend(refresh)
    cofog_spending_detail, cofog_spending_detail_updated = load_cofog_spending_detail(refresh)
    ...
    peer_tax_history, peer_tax_history_updated = load_peer_tax_pressure_history(refresh)
    peer_spending_history, peer_spending_history_updated = load_peer_spending_history(refresh, "TOTAL", "total")
    peer_social_history, peer_social_history_updated = load_peer_spending_history(refresh, "GF10", "gf10")
    peer_spending_functions_history, peer_spending_functions_history_updated = load_peer_spending_functions_history(refresh)
    ...
    oecd_revenue, oecd_revenue_peers, oecd_revenue_history, oecd_revenue_peer_history = load_oecd_revenue_data(refresh)
    oecd_spending, oecd_spending_peers, oecd_spending_history, oecd_spending_peer_history = load_oecd_spending_data(refresh)
    tipo_reddito, calcolo_irpef = load_declaration_data(refresh)
    regional_budgets = enrich_regional_budgets(load_regional_budgets(refresh), refresh, spending_adjustments)
    siope_flows = load_siope_flows(refresh, adjustments=spending_adjustments)
    ...
    export_bilancio_source_json(
        ...
        peer_tax_history=peer_tax_history,
        peer_spending_history=peer_spending_history,
        peer_social_history=peer_social_history,
        peer_spending_functions_history=peer_spending_functions_history,
        source_updates={
    ...
    append_siope_flows_to_source_json(siope_flows)
