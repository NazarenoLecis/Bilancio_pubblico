...
def records_from_frame(frame, include_index=False, index_name="year"):
    ...
    for row in source.to_dict(orient="records"):
        ...
        for key, value in row.items():
            if isinstance(value, (int, float)):
                clean_row[key] = to_number(value, 6)
            elif pd.isna(value):
                clean_row[key] = None
            else:
...
def to_payload_item(code, label, year, value, source, unit, status=None, note=None):
    payload = {
        "code": code,
        "label": label,
        "source": source,
...
def revenue_group(label, source):
...
def is_revenue_line_candidate(label):
    if not label:
        return False
...
def all_revenue_lines_from_items(erariali_items, erariali_months, territoriali_items, territoriali_months):
    rows = []
    seen = set()
    for source, items, months in (
        ...
        for item in items:
            ...
            if not is_revenue_line_candidate(label):
                continue
            try:
            ...
            latest_value = to_number(annual.loc[latest_year], 4)
            ...
            if key in seen:
                continue
            seen.add(key)
            code = slug_code(f"{source}_{label}")
            rows.append(
                {
                    "code": code,
                    "label": label,
                    "group": revenue_group(label, source),
                    "source": source,
                    "unit": "mld",
                    "latest_year": to_number(latest_year),
                    "latest_value_mld": latest_value,
                    "series": [
                        {"year": to_number(int(year)), "value_mld": to_number(value, 4)}
...
def build_tax_items_detail(
    ...
    for code, label, source, selector in [
        ...
        try:
            year, value = safe_mef_value(mef_items, mef_months, selector)
    ...
    try:
        imu_year, imu_value = safe_mef_value(territoriali_items, territoriali_months, {"exact": "Imu - Imis (Quota Comuni)"})
        rows.append(
            to_payload_item(
                "IMU",
                "IMU (quota Comuni)",
    ...
    if succession_value is not None:
        rows.append(
            to_payload_item(
                ...
                succession_year,
                to_number(succession_value / 1000.0, 4),
...
def build_spending_focus(cofog_spending, total_spending):
    focus = []
    ...
    if total_spending is not None and not total_spending.empty and latest_year in total_spending.index:
        ...
        latest_total = safe_last(total_row.get("mld"))
    ...
    for code, label, cofog_code, share_code_unused, is_proxy in requested:
        if cofog_code is not None and cofog_code in latest_cofog.index:
            row = latest_cofog.loc[cofog_code]
            value_mld = to_number(row["mld"], 4)
            value_pil = to_number(row["pil"], 4)
            payload = to_payload_item(
                code,
                label,
                latest_year,
                value_mld,
            ...
            if latest_total:
                payload["share_spesa_totale"] = to_number(value_mld / latest_total * 100.0 if latest_total else None, 4)
            if is_proxy:
...
def cofog_summary_with_share(cofog_spending, total_spending):
    if cofog_spending is None or cofog_spending.empty:
        return []
    ...
    for index, row in latest.iterrows():
        mld = to_number(row["mld"], 4)
        share = to_number(mld / total * 100.0 if total else None, 4)
        item = {
            "code": row["codice"],
            "label": row["funzione"],
            "year": to_number(row["anno"]),
            "value_mld": mld,
            "value_pil": to_number(row["pil"], 4),
        }
...
def build_declaration_distributions(calcolo_irpef):
    by_band, share_by_band = build_decl_bands(calcolo_irpef)
    ...
    return {
        ...
        "totals": {
            "contributors": safe_last(total_contributors),
            "tax_mld": safe_last(total_tax_mld),
        },
        "taxpayers_focus": [
...
def main_taxes_rows(erariali_items, erariali_months, territoriali_items, territoriali_months):
    candidates = [
    ...
    for label, items, months, selector, code in candidates:
        try:
            ...
            rows.append({
                ...
                "label": label,
                "year": to_number(year),
                "value": to_number(annual.loc[year], 4),
                "unit": "mld",
            })
        except Exception:
            continue

    rows = [row for row in rows if safe_last(row["year"]) is not None and safe_last(row["value"]) is not None]
    rows = sorted(rows, key=lambda item: item["value"], reverse=True)
    return rows[:10]


def tax_pressure_trend(tax_pressure):
    if tax_pressure is None or tax_pressure.empty:
        return []
    total = tax_pressure.sum(axis=1).sort_index()
    return [
        {"year": to_number(year), "value": to_number(value, 3), **{
            col.lower(): to_number(tax_pressure.loc[year, col], 4) for col in tax_pressure.columns
        }}
        for year, value in total.items()
    ]


def gdp_mld_by_year(total_spending):
    if total_spending is None or total_spending.empty:
        return {}
    gdp = {}
    for year, row in total_spending.sort_index().iterrows():
        spending_mld = to_number(row.get("value"), 6)
        if spending_mld is None:
            spending_mld = to_number(row.get("mld"), 6)
        spending_pil = to_number(row.get("value_pil_percent"), 6)
        if spending_pil is None:
            spending_pil = to_number(row.get("value_pil"), 6)
        if spending_pil is None:
            spending_pil = to_number(row.get("pil_percent"), 6)
        if spending_pil is None:
            spending_pil = to_number(row.get("pil"), 6)
        if spending_mld and spending_pil:
            gdp[int(year)] = spending_mld / spending_pil * 100.0
    return gdp


def build_social_contributions_item(tax_pressure, total_spending):
    if tax_pressure is None or tax_pressure.empty:
        return None
    column = next(
    ...
    if column is None:
        return None
    gdp_by_year = gdp_mld_by_year(total_spending)
    series = []
    for year, percent_pil in tax_pressure[column].dropna().sort_index().items():
        ...
        row = {
            "year": to_number(year_int),
            "percent_pil": to_number(percent_pil, 4),
        }
        if value_mld is not None:
            row["value_mld"] = to_number(value_mld, 4)
        series.append(row)
...
def prepend_social_contributions(rows, social_contributions):
...
def build_peer_rows(peer_tax, peer_spending, peer_social):
    by_code = {}
    if peer_tax is not None and not peer_tax.empty:
        for index, row in peer_tax.iterrows():
            code = row.get("codice")
            if code is None:
                continue
            item = by_code.setdefault(code, {"code": str(code), "country": row.get("paese") or str(code)})
            item["tax_pressure"] = to_number(row.get("valore"), 3)
            item["tax_year"] = to_number(row.get("anno"))
    if peer_spending is not None and not peer_spending.empty:
        for index, row in peer_spending.iterrows():
            code = row.get("codice")
            if code is None:
                continue
            item = by_code.setdefault(code, {"code": str(code), "country": row.get("paese") or str(code)})
            item["public_spending"] = to_number(row.get("valore"), 3)
            item["spending_year"] = to_number(row.get("anno"))
    if peer_social is not None and not peer_social.empty:
        for index, row in peer_social.iterrows():
            code = row.get("codice")
            if code is None:
                continue
            item = by_code.setdefault(code, {"code": str(code), "country": row.get("paese") or str(code)})
            item["social_spending"] = to_number(row.get("valore"), 3)
            item["social_year"] = to_number(row.get("anno"))
    return list(by_code.values())


def build_peer_spending_function_options(peer_spending_functions):
    ...
    return [
        {
            ...
            "level": to_number(row["cofog_level"]),
            "parent_code": safe_last(row.get("parent_code")),
            "parent_label": safe_last(row.get("parent_label")),
            "unit": "% PIL",
...
def build_peer_spending_function_rows(peer_spending_functions):
    if peer_spending_functions is None or peer_spending_functions.empty:
        return []
    rows = []
        ...
        rows.append(
            {
                ...
                "year": to_number(row.get("anno")),
                "value": to_number(row.get("valore"), 4),
                "unit": "% PIL",
                ...
                "cofog_level": to_number(row.get("cofog_level")),
                "parent_code": safe_last(row.get("parent_code")),
                "parent_label": safe_last(row.get("parent_label")),
            }
        )
...
def peer_metric_series_rows(frame, metric_id, metric_label):
    if frame is None or frame.empty:
        return []
    rows = []
    selected = frame.sort_values(["codice", "anno"])
    for index, row in selected.iterrows():
        rows.append(
            {
                "metric_id": metric_id,
...
def combine_peer_metric_series(*items):
    rows = []
    for item in items:
        rows.extend(item or [])
    return rows
...
def cofog_distribution(cofog_spending):
    if cofog_spending is None or cofog_spending.empty:
        return []
    latest_year = int(cofog_spending["anno"].max())
...
def safe_annual_series_from_selectors(items, months, selectors):
...
def build_revenue_category_series(erariali_items, erariali_months, territoriali_items, territoriali_months):
    candidates = [
        {
            "code": "IRPEF",
            "label": "IRPEF",
    ...
    for candidate in candidates:
        ...
        series = safe_annual_series_from_selectors(items, months, candidate["selectors"])
        ...
        latest_value = to_number(valid.loc[latest_year], 4)
        row = {
            "code": candidate["code"],
            "label": candidate["label"],
            "group": candidate["group"],
...
def cofog_category_series(cofog_spending):
    if cofog_spending is None or cofog_spending.empty:
        return []
    ...
    for code, group in grouped:
        ...
        latest = {
            "code": code,
            "label": safe_last(last_row["funzione"]) or code,
            "unit": "mld",
            "series": records,
            "latest_year": to_number(int(last_row["anno"])),
            "latest_value_mld": to_number(last_row["mld"], 4),
        }
        if records:
...
def cofog_detail_series(cofog_spending_detail):
    ...
    for code, group in grouped:
        ...
        item = {
            "code": code,
            "label": safe_last(last_row["funzione"]) or code,
            "parent_code": safe_last(last_row["parent_code"]) or str(code)[:4],
            "parent_label": safe_last(last_row["parent_function"]) or str(code)[:4],
            "unit": "mld",
            "series": records,
            "latest_year": to_number(int(last_row["anno"])),
            "latest_value_mld": to_number(last_row["mld"], 4),
            "latest_value_pil": to_number(last_row["pil"], 4),
        }
        if records:
            payload.append(item)
    ...
    for item in payload:
        ...
        item["latest_share_parent_percent"] = (
            to_number((item.get("latest_value_mld") or 0) / parent_total * 100.0, 4)
            if parent_total
            else None
        )
...
def build_pie_payload(rows, value_key, year_key="latest_year"):
    normalized = [row for row in rows if row and safe_last(row.get(value_key)) is not None and safe_last(row.get(year_key)) is not None]
    total = safe_sum(*[row.get(value_key) for row in normalized])
    result = []
    for row in normalized:
        item_value = row.get(value_key)
        share = to_number(item_value / total * 100.0 if total else None, 3)
        result.append(
            {
                "code": row.get("code"),
                "label": row.get("label"),
                "year": row.get(year_key),
                "value_mld": to_number(item_value, 4),
                "share_percent": share,
            }
        )
...
def build_under_500m_revenue_summary(revenue_lines, total_revenue_year=None, threshold_mld=0.5):
    ...
    return {
        ...
        "threshold_mld": threshold_mld,
        "entries": candidates,
        "entries_count": safe_count([row["value_mld"] for row in candidates]),
        "under_500_total_mld": to_number(threshold_total, 4),
        "under_500_share_of_total_percent": to_number(threshold_total / total_revenue_year * 100.0 if total_revenue_year else None, 4),
...
def build_succession_payload(total_erariali):
    try:
    ...
    for row in SUCCESSIONI_DONAZIONI_SERIE:
        rows.append(
            {
                "year": to_number(row["anno"]),
                "value_million_euro": to_number(row["milioni"], 4),
            }
        )
    return {
        ...
        "share_of_2025_revenue_percent": to_number(share, 4),
    }


def revenue_mix_2025(mef_items, mef_months, territoriali_items, territoriali_months):
    def safe_mld(items, months, selector):
        selected = find_mef_item(items, **selector)
        annual = mef_annual_series(selected, months)
        year = annual.last_valid_index()
        return to_number(annual.loc[year], 4) if year is not None else None
    ...
    rows = [
        {
            ...
            "value_mld": safe_sum(
                safe_mld(mef_items, mef_months, {"exact": "IRPEF"}),
                safe_mld(territoriali_items, territoriali_months, {"starts": "Addizionale regionale IRPEF"}),
                safe_mld(territoriali_items, territoriali_months, {"starts": "Addizionale comunale IRPEF"}),
            ),
        },
        {
            "code": "CONSUMI",
            "label": "Consumi",
            "value_mld": safe_sum(
                safe_mld(mef_items, mef_months, {"exact": "IVA"}),
                safe_mld(mef_items, mef_months, {"starts": "Accisa sui prodotti energetici"}),
                safe_mld(mef_items, mef_months, {"starts": "Accisa sul gas naturale"}),
                safe_mld(mef_items, mef_months, {"starts": "Accisa sull'energia elettrica"}),
                safe_mld(mef_items, mef_months, {"starts": "Imposta sul consumo dei tabacchi"}),
            ),
        },
        {
            "code": "IMPRESE",
            "label": "Imprese",
            "value_mld": safe_sum(
                safe_mld(mef_items, mef_months, {"exact": "IRES"}),
                safe_mld(territoriali_items, territoriali_months, {"exact": "IRAP"}),
            ),
        },
    ]
    rows = [row for row in rows if safe_last(row["value_mld"]) is not None]
    return sorted(rows, key=lambda item: item["value_mld"], reverse=True)


def build_direct_indirect(erariali_items, erariali_months):
    try:
        ...
        return records_from_frame(frame.loc[2002:2025], include_index=True, index_name="year")
    except Exception:
        return []


def build_decl_bands(calcolo_irpef):
    ...
    for band, row in grouped.iterrows():
        tax = to_number(row[IRPEF_TAX_LABEL], 4)
        contrib = to_number(row[IRPEF_CONTRIBUTORS_LABEL], 4)
        by_band.append(
            {
                "band": band,
                "contributors": contrib,
                "tax_eur": to_number(tax),
                "tax_mld": to_number(tax / 1_000_000_000 if safe_last(tax) else None, 4),
                "average_tax_per_contributor": to_number(
        ...
        shares.append(
            {
                "band": band,
                "tax_share": to_number(tax / total_tax * 100.0 if total_tax else None, 3),
                "contributors_share": to_number(contrib / total_contrib * 100.0 if total_contrib else None, 3),
            }
        )
    return by_band, shares


def build_income_share(tipo_reddito):
    columns = [
    ...
    for band, row in grouped.iterrows():
        rows.append(
            {
                "band": band,
                "work": to_number(row["Reddito da lavoro dipendente e assimilati - Ammontare in euro"] / 1_000_000_000, 4),
                "capital_gain": to_number(row["Plusvalenze di natura finanziaria - Ammontare in euro"] / 1_000_000_000, 4),
                "capital_income": to_number(row["Reddito di capitale (sez. IA e IB) - Ammontare in euro"] / 1_000_000_000, 4),
                "total_mld": to_number(
                    (
...
def oecd_peer_series_records(frames, code):
    frame = frames.get(code, pd.DataFrame()) if isinstance(frames, dict) else pd.DataFrame()
    return records_from_frame(frame)


def build_oecd_payload(
    oecd_revenue,
    oecd_revenue_peers,
    oecd_revenue_history,
    oecd_revenue_peer_history,
    oecd_spending,
    oecd_spending_peers,
    oecd_spending_history,
    oecd_spending_peer_history,
):
    return {
        "revenue_categories": records_from_frame(oecd_revenue),
        "spending_categories": records_from_frame(oecd_spending),
        "peer_revenue_total": records_from_frame(oecd_revenue_peers.get("_T", pd.DataFrame())),
        "peer_spending_total": records_from_frame(oecd_spending_peers.get("_T", pd.DataFrame())),
        "peer_inheritance": records_from_frame(oecd_revenue_peers.get("T_4300", pd.DataFrame())),
        "revenue_category_series": records_from_frame(oecd_revenue_history),
        "spending_category_series": records_from_frame(oecd_spending_history),
        "peer_revenue_total_series": oecd_peer_series_records(oecd_revenue_peer_history, "_T"),
        "peer_spending_total_series": oecd_peer_series_records(oecd_spending_peer_history, "_T"),
        "peer_inheritance_series": oecd_peer_series_records(oecd_revenue_peer_history, "T_4300"),
        "peer_revenue_category_series": {
            code: oecd_peer_series_records(oecd_revenue_peer_history, code)
            for code in sorted(oecd_revenue_peer_history or {})
        },
        "peer_spending_category_series": {
            code: oecd_peer_series_records(oecd_spending_peer_history, code)
            for code in sorted(oecd_spending_peer_history or {})
        },
    }
...
def export_bilancio_source_json(
    ...
    peer_tax_history=None,
    peer_spending_history=None,
    peer_social_history=None,
    peer_spending_functions_history=None,
    source_updates=None,
    manifest_rows=None,
):
    ...
    latest_pressure_year = to_number(latest_pressure.index.max()) if not latest_pressure.empty else None
    latest_pressure_value = to_number(latest_pressure.iloc[-1], 3) if not latest_pressure.empty else None
    ...
    pressure_rows = tax_pressure_trend(tax_pressure)
    social_contributions = build_social_contributions_item(tax_pressure, total_spending)
    top_taxes = prepend_social_contributions(
        main_taxes_rows(mef_items, mef_months, territoriali_items, territoriali_months),
        social_contributions,
    )
    peer_rows = build_peer_rows(peer_tax, peer_spending, peer_social)
    peer_spending_function_options = build_peer_spending_function_options(peer_spending_functions)
    peer_spending_function_rows = build_peer_spending_function_rows(peer_spending_functions)
    peer_tax_series = peer_metric_series_rows(peer_tax_history, "tax_pressure", "Pressione fiscale e contributiva")
    peer_spending_series = peer_metric_series_rows(peer_spending_history, "public_spending", "Spesa pubblica totale")
    peer_social_series = peer_metric_series_rows(peer_social_history, "social_spending", "Spesa per protezione sociale")
    peer_metric_series = combine_peer_metric_series(peer_tax_series, peer_spending_series, peer_social_series)
    peer_spending_function_series = build_peer_spending_function_rows(peer_spending_functions_history)
    ...
    irpef_by_band, irpef_share_by_band = build_decl_bands(calcolo_irpef)
    income_by_band = build_income_share(tipo_reddito)
    direct_indirect = build_direct_indirect(mef_items, mef_months)
    revenue_mix = revenue_mix_2025(mef_items, mef_months, territoriali_items, territoriali_months)
    try:
    ...
    succession_payload = build_succession_payload(total_erariali_2025 if not pd.isna(total_erariali_2025) else None)
    revenue_tax_items = build_tax_items_detail(
        mef_items,
        mef_months,
        territoriali_items,
        territoriali_months,
    ...
    revenue_category_series = build_revenue_category_series(
        mef_items,
        mef_months,
        territoriali_items,
        territoriali_months,
    )
    all_revenue_lines = all_revenue_lines_from_items(
        mef_items,
        mef_months,
        territoriali_items,
        territoriali_months,
    )
    revenue_category_series = prepend_social_contributions(revenue_category_series, social_contributions)
    all_revenue_lines = prepend_social_contributions(all_revenue_lines, social_contributions)
    revenue_pie = build_pie_payload(revenue_category_series, "latest_value_mld")
    spending_category_series = cofog_category_series(cofog_spending_trend)
    spending_function_detail_series = cofog_detail_series(cofog_spending_detail)
    spending_pie = build_pie_payload(spending_category_series, "latest_value_mld")
    under_500m_revenue = build_under_500m_revenue_summary(
        all_revenue_lines,
        total_revenue_year=to_number(total_erariali_2025, 4),
    )
    spending_focus = build_spending_focus(cofog_spending, total_spending)
    cofog_summary = cofog_summary_with_share(cofog_spending, total_spending)
    declaration_summary = build_declaration_distributions(calcolo_irpef)
    ...
    payload = {
        "meta": {
            ...
            "source_updates": {
                "mef_entrate": safe_last(mef_data.get("aggiornamento")),
                "mef_territoriali": safe_last(territoriali_data.get("aggiornamento")),
        ...
        "kpis": [
            ...
            },
            {
                ...
                "value": to_number(latest_spending_pil, 3),
                "unit": "% PIL",
                "year": to_number(latest_spending_year),
                "value_mld": to_number(latest_spending_mld, 3),
                "unit_mld": "mld",
            },
            {
                "id": "social_spending",
        ...
        "peer": peer_rows,
        "peer_series": peer_metric_series,
        "peer_tax_pressure_series": peer_tax_series,
        "peer_public_spending_series": peer_spending_series,
        "peer_social_spending_series": peer_social_series,
        "peer_spending_function_options": peer_spending_function_options,
        "peer_spending_functions": peer_spending_function_rows,
        "peer_spending_functions_series": peer_spending_function_series,
        ...
        "known_revenue_gaps": known_revenue_gaps(),
        "declaration_summary": declaration_summary,
        "cofog_spending_2024": cofog_distribution(cofog_spending),
        "spending_by_function": cofog_summary,
        "spending_focus": spending_focus,
        "cofog_spending_trend": records_from_frame(cofog_spending_trend),
        "cofog_spending_detail": records_from_frame(cofog_spending_detail),
        ...
        "total_spending_trend": records_from_frame(total_spending),
        ...
        "household_wealth_distribution": build_wealth_payload(),
        "succession_gift_tax": succession_payload,
        "oecd": build_oecd_payload(
            oecd_revenue,
            oecd_revenue_peers,
            oecd_revenue_history,
            oecd_revenue_peer_history,
        ...
        "counts": {
            ...
            "peer_series": len(peer_metric_series),
            "peer_spending_functions": len(peer_spending_function_rows),
            "peer_spending_functions_series": len(peer_spending_function_series),
