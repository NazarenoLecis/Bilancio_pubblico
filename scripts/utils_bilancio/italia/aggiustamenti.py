...
from utils_bilancio.generali.costanti import SOURCE_DATA_JSON_PATH, SOURCE_EUROSTAT_EXP


SOURCE_EUROSTAT_POPULATION = "Fonte: Eurostat demo_pjan, popolazione residente al 1 gennaio, totale"
SOURCE_EUROSTAT_HICP = "Fonte: Eurostat prc_hicp_aind, HICP all-items, indice medio annuo 2015=100"
...
def load_spending_adjustments(refresh=False):
    ...
    hicp, hicp_updated, hicp_error = safe_eurostat_series(
        "prc_hicp_aind",
        {
            "format": "JSON",
            "lang": "en",
        ...
        "eurostat_prc_hicp_aind_IT_cp00_inx_a_avg.json",
        refresh,
    )
...
def latest_common_price_year(hicp, years):
...
def augment_row_values(row, population, hicp, base_year, base_price_index):
...
def augment_spending_frame(frame, adjustments):
    ...
    if frame is None or frame.empty:
        return frame
    population = adjustments.get("population", empty_series())
    hicp = adjustments.get("hicp", empty_series())
    result = frame.copy()
    base_year = latest_common_price_year(hicp, result["anno"].dropna().astype(int).unique())
    base_price_index = hicp.get(base_year) if base_year is not None else None
    return result.apply(
        lambda row: augment_row_values(row, population, hicp, base_year, base_price_index),
        axis=1,
    )
...
def append_spending_adjustments_to_source_json(adjustments):
    ...
    for note in (
        ...
        append_unique(method_notes, note)
    ...
    payload["spending_adjustment_sources"] = {
        ...
        "population": series_records(adjustments.get("population")),
        "hicp": series_records(adjustments.get("hicp")),
