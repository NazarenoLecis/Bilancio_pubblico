...
from bilancio_pubblico.utils import SOURCE_DATA_JSON_PATH, fetch_json
...
def _fet_url(endpoint, params):
    return f"{OPENBDAP_FET_API_BASE_URL}/{endpoint}?{urlencode(params)}"


def _fetch_fet(endpoint, params, refresh):
    query = urlencode(sorted(params.items()))
    cache_hash = hashlib.sha1(query.encode("utf-8")).hexdigest()[:16]
    cache_name = f"openbdap_fet_{endpoint}_{cache_hash}.json"
    data = fetch_json(_fet_url(endpoint, params), cache_name, refresh)
    if data is None:
        return []
    return data


def _region_spec(year, compartment, entrate_spese, mission="00", title="00", tipologia="00", measure="spesa"):
    return {
...
def _rows_from_response(data, year, compartment, measure, extra=None):
    rows = []
    extra = extra or {}
    for item in data or []:
        region = _canonical_region(item.get("region_name"))
        value = _to_float(item.get("itamvalue"))
...
def _load_region_endpoint_rows(refresh, specs):
    rows = []
    datasets = []
    errors = []
    for spec in specs:
        params = spec["params"]
        try:
            data = _fetch_fet(spec["endpoint"], params, refresh)
...
def _balance_frame(spending, revenue):
...
def load_regional_budgets(refresh=False):
    ...
    for year in FET_TOTAL_YEARS:
        specs.append({**_region_spec(year, DEFAULT_COMPARTMENT, "1", measure="spesa"), "dataset": "spending_by_region", "label": "Spese per regione"})
        specs.append({**_region_spec(year, DEFAULT_COMPARTMENT, "2", measure="entrate"), "dataset": "revenue_by_region", "label": "Entrate per regione"})

    for year in FET_DETAIL_YEARS:
        for code, label in MISSION_LABELS.items():
            specs.append(
                {
                    **_region_spec(year, DEFAULT_COMPARTMENT, "1", mission=code, measure="spesa"),
        ...
        for code, label in SPENDING_TITLE_LABELS.items():
            specs.append(
                {
                    **_region_spec(year, DEFAULT_COMPARTMENT, "1", title=code, measure="spesa"),
        ...
        for code, label in REVENUE_TITLE_LABELS.items():
            specs.append(
                {
                    **_region_spec(year, DEFAULT_COMPARTMENT, "2", title=code, measure="entrate"),
    ...
    frame, datasets, errors = _load_region_endpoint_rows(refresh, specs)
    if frame.empty:
    ...
    balances_by_region = _balance_frame(spending_by_region, revenue_by_region)
    ...
    return {
        ...
        "datasets": datasets,
        "errors": errors,
        "coverage": _coverage(spending_by_region),
