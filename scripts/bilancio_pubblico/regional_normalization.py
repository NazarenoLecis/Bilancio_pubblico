...
from bilancio_pubblico.regional_denominators import (
...
def enrich_regional_budgets(regional_budgets, refresh=False):
    denominators = load_regional_denominators(refresh)
    for key in NORMALIZED_REGIONAL_KEYS:
        regional_budgets[key] = add_regional_denominators(regional_budgets.get(key), denominators)
